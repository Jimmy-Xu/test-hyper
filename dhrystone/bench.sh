...
JQ=${BASE_DIR}/../../util/jq
DHRYSTONE_REPO="git@github.com:Jimmy-Xu/dhrystone-deb.git"
DHRYSTONE_DIR="dhrystone-deb"
DHRYSTONE_BIN="dhry"
DHRYSTONE_PATH="${BASE_DIR}/${DHRYSTONE_DIR}/${DHRYSTONE_BIN}"
...
DOCKER_IMAGE="hyper:dhrystone"
POD_FILENAME="hyper-dhrystone"
...
function generate_test_case() {
  ...
  if [ ${EXEC_MODE} == "live"  ];then
    ...
    TEST_CPU_CASE=( 100000000  200000000 300000000 400000000 500000000 )
  else
    #test case for dev(fast test)
    TEST_CPU_CASE=( 10000000  20000000 30000000 40000000 50000000 )
  fi
}
...
function build_dhrystone() {
  if [ -d "${BASE_DIR}/${DHRYSTONE_DIR}" ];then
    title "Clear old dhrystone in host os"
    echo "clear old '${BASE_DIR}/${DHRYSTONE_DIR}'"
    rm "${BASE_DIR}/${DHRYSTONE_DIR}" -rf
  fi
  if [ ! -d "${BASE_DIR}/${DHRYSTONE_DIR}" ];then
    title "Installing dhrystone in host os"
    git clone ${DHRYSTONE_REPO}
    cd ${BASE_DIR}/${DHRYSTONE_DIR}
    git checkout mine
    make
    if [ -f ${DHRYSTONE_PATH} ];then
      echo "build dhrystone success"
    else
      echo "build dhrystone failed"
      exit 1
    fi
  else
    echo "remove old dhrystone failed"
    exit 1
  fi
}
...
function do_cpu_test() {
  ...
  do
    ...
    _NUMBER_OF_RUNS=${TEST_PARAM[0]}

    TEST_CMD="$(cputest_cmd ${_NUMBER_OF_RUNS})"
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "host" &>/dev/null);then
      ...
      show_test_cmd  " [ bash -c \"${BASE_DIR}/${TEST_CMD}\" ]${WHITE}"
      if [ "${DRY_RUN}" != "true" ];then
        bash -c "${BASE_DIR}/${TEST_CMD}"
      fi
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "docker" &>/dev/null);then
      ...
      echo "test_case: ${test_case// /-}"
      show_test_cmd  " [ docker run -t --memory=${MEMORY_SIZE}m --cpuset-cpus=${CPU_SET// /,} ${DOCKER_IMAGE} /root/${TEST_CMD} ]${WHITE}"
      if [ "${DRY_RUN}" != "true" ];then
        docker run -t --memory=${MEMORY_SIZE}m --cpuset-cpus=${CPU_SET// /,} ${DOCKER_IMAGE} /root/${TEST_CMD}
      fi
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "hyper" &>/dev/null);then
      ...
      if [ ${USE_HYPER_RUN} == "true" ];then
        show_test_cmd  " [ sudo hyper run --cpu=${CPU_NUM} --memory=${MEMORY_SIZE} ${DOCKER_IMAGE} /root/${TEST_CMD} ]${WHITE}"
        if [ "${DRY_RUN}" != "true" ];then
          sudo hyper run --cpu=${CPU_NUM} --memory=${MEMORY_SIZE} ${DOCKER_IMAGE} /root/${TEST_CMD}
        fi
      else
        CONTAINER_ID=$(hyper_get_container_id)
        if [ "${CONTAINER_ID}" == " " ];then
        ...
        show_test_cmd  " [ sudo hyper exec ${CONTAINER_ID} /root/${TEST_CMD} ]${WHITE}"
        if [ "${DRY_RUN}" != "true" ];then
          sudo hyper exec ${CONTAINER_ID} /root/${TEST_CMD}
        fi
      fi
