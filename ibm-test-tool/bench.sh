...
function generate_test_case() {
  ...
  if [ ${EXEC_MODE} == "live"  ];then
    ...
    TEST_CPU_CASE=( "${MAX_REQUESTS} 95 1 50000"  "${MAX_REQUESTS} 95 ${CPU_NUM} 50000" )
    ...
    TEST_IO_CASE=( "${MAX_REQUESTS} 95 1 2G $((512*1)) 5"  "${MAX_REQUESTS} 95 ${CPU_NUM} 2G $((64*1024)) 5" )
  else
    #test case for dev(fast test)
    #--max-requests(10000*), --percentile(95*), --num-threads(1*), --cpu-max-prime
    TEST_CPU_CASE=( "${MAX_REQUESTS} 95 1 5000"  "${MAX_REQUESTS} 95 ${CPU_NUM} 5000" )

    #--max-requests(10000*), --percentile(95*), --num-threads(1*), --memory-scope(global*|local), --memory-total-size(100G*), --memory-block-size(1K*)
    TEST_MEM_CASE=( 1 2 )

    #--max-requests(10000*), --percentile(95*), --num-threads(1*), --file-total-size(2G), --file-block-size(16384*), --file-num(128*)
    TEST_IO_CASE=( "${MAX_REQUESTS} 95 1 4M $((16*1024)) 2"  "${MAX_REQUESTS} 95 ${CPU_NUM} 4M $((16*1024)) 2" )
  fi
}
...
function clone_kvm_docker_comparison(){
  title "Clone kvm-docker-comparison"
  if [ ! -d kvm-docker-comparison ];then
    git clone https://github.com/Jimmy-Xu/kvm-docker-comparison.git
  else
    echo "dir kvm-docker-comparison exist, skip!"
  fi
}
...
function build_dockerfile() {
  ...
  if [ ! -d kvm-docker-comparison ];then
    echo "please clone kvm-docker-comparison first!"
    exit 1
  fi
...
function do_memory_test() {
  ...
  do
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "host" &>/dev/null);then
      ...
      echo "test_case: ${test_case// /-}"
      show_test_cmd  " [ bash -c \"export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd ${BASE_DIR}/STREAM;${TEST_CMD}\" ]${WHITE}"
      echo "DRY_RUN:${DRY_RUN}"
      if [ "${DRY_RUN}" != "true" ];then
        bash -c "export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd ${BASE_DIR}/STREAM;${TEST_CMD}"
      fi
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "docker" &>/dev/null);then
      ...
      echo "test_case: ${test_case// /-}"
      show_test_cmd  " [ docker run -t --memory=${MEMORY_SIZE}m --cpuset-cpus=${CPU_SET// /,} ${DOCKER_IMAGE} bash -c \"export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd /root/STREAM;${TEST_CMD}\" ]${WHITE}"
      if [ "${DRY_RUN}" != "true" ];then
        docker run -t --memory=${MEMORY_SIZE}m --cpuset-cpus=${CPU_SET// /,} ${DOCKER_IMAGE} bash -c "export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd /root/STREAM;${TEST_CMD}"
      fi
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "hyper" &>/dev/null);then
      ...
      echo "test_case: ${test_case// /-}"
      if [ ${USE_HYPER_RUN} == "true" ];then
        show_test_cmd  " [ sudo hyper run --cpu=${CPU_NUM} --memory=${MEMORY_SIZE} ${DOCKER_IMAGE} bash -c \"export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd /root/STREAM;${TEST_CMD}\" ]${WHITE}"
        if [ "${DRY_RUN}" != "true" ];then
          sudo hyper run --cpu=${CPU_NUM} --memory=${MEMORY_SIZE} ${DOCKER_IMAGE} bash -c "export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd /root/STREAM;${TEST_CMD}"
        fi
      else
        CONTAINER_ID=$(hyper_get_container_id)
        if [ "${CONTAINER_ID}" == " " ];then
        ...
        show_test_cmd  " [ sudo hyper exec ${CONTAINER_ID} bash -c \"export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd /root/STREAM;${TEST_CMD}\" ]${WHITE}"
        if [ "${DRY_RUN}" != "true" ];then
          sudo hyper exec ${CONTAINER_ID} bash -c "export OMP_NUM_THREADS=${_OMP_NUM_THREADS}; cd /root/STREAM;${TEST_CMD}"
        fi
      fi
...
function do_io_test() {
  ...
  for (( i = 0 ; i < ${#TEST_IO_CASE[@]} ; i++ ))
  do
    #get test parameter
    test_case=${TEST_IO_CASE[$i]}
    TEST_PARAM=(${test_case})
    if [ ${#TEST_PARAM[@]} -ne ${PARAM_NUM} ];then
      echo "Parameter number should be ${PARAM_NUM}, but current is ${#TEST_PARAM[@]}"
      echo "current parameter: ${test_case}"
