...
JQ=${BASE_DIR}/../../util/jq
CLASSIC_BENCHMARKS_REPO="git@github.com:Jimmy-Xu/classic_benchmarks.git"
CLASSIC_BENCHMARKS_DIR="classic_benchmarks"
...
DOCKER_IMAGE="hyper:classic_benchmarks"
POD_FILENAME="hyper-classic_benchmarks"
...
function build_classic_benchmarks() {
  if [ -d "${BASE_DIR}/${CLASSIC_BENCHMARKS_DIR}" ];then
    title "Clear old classic_benchmarks in host os"
    echo "clear old '${BASE_DIR}/${CLASSIC_BENCHMARKS_DIR}'"
    rm "${BASE_DIR}/${CLASSIC_BENCHMARKS_DIR}" -rf
  fi
  if [ ! -d "${BASE_DIR}/${CLASSIC_BENCHMARKS_DIR}" ];then
    title "Installing classic_benchmarks in host os"
    git clone ${CLASSIC_BENCHMARKS_REPO}
    cd ${BASE_DIR}/${CLASSIC_BENCHMARKS_DIR}/source_code
    make
    if [ -f dhry1 -a -f dhry2 -a -f whets -a -f lpack ];then
      echo "build classic_benchmarks success"
    else
      echo "build classic_benchmarks failed"
      exit 1
    fi
  else
    echo "remove old classic_benchmarks failed"
    exit 1
  fi
}
...
function start_test() {
  ...
  #3 start test
  if (echo "${TEST_ITEM[@]}" | grep -w "dhry2" &>/dev/null);then
    do_cpu_test "${TEST_TARGET}" "dhry2"
  fi
  if (echo "${TEST_ITEM[@]}" | grep -w "whets" &>/dev/null);then
    do_cpu_test "${TEST_TARGET}" "whets"
  fi
  if (echo "${TEST_ITEM[@]}" | grep -w "lpack" &>/dev/null);then
    do_cpu_test "${TEST_TARGET}" "lpack"
  fi
  TEST_END_TS=$( date +"%s" )
...
function do_cpu_test() {
  ...
  do
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "host" &>/dev/null);then
      ...
      mkdir -p ${BASE_DIR}/log/host
      case "${TEST_METHOD}" in
        dhry2)  rm ${BASE_DIR}/Dhry.txt -f >/dev/null 2>&1
                rm ${BASE_DIR}/log/host/Dhry.txt -f >/dev/null 2>&1
            ;;
        whets)  rm ${BASE_DIR}/whets.txt -f >/dev/null 2>&1
                rm ${BASE_DIR}/log/host/whets.txt -f >/dev/null 2>&1
            ;;
        lpack)  rm ${BASE_DIR}/lpack.txt -f >/dev/null 2>&1
                rm ${BASE_DIR}/log/host/lpack.txt -f >/dev/null 2>&1
            ;;
        *) echo "unknown method: ${TEST_METHOD}"; exit 1 ;;
      esac
      show_test_cmd  " [ bash -c \"${BASE_DIR}/test/${TEST_CMD} ${TEST_METHOD}}\" ]${WHITE}"
      if [ "${DRY_RUN}" != "true" ];then
        bash -c "${BASE_DIR}/test/${TEST_CMD} ${TEST_METHOD}"
      fi
      if [ "${DRY_RUN}" != "true" ];then
        case "${TEST_METHOD}" in
          dhry2)  if [ -f ${BASE_DIR}/Dhry.txt ];then mv ${BASE_DIR}/Dhry.txt ${BASE_DIR}/log/host/; else echo "dhry test failed";exit 1; fi
              ;;
          whets)  if [ -f ${BASE_DIR}/whets.txt ];then mv ${BASE_DIR}/whets.txt ${BASE_DIR}/log/host/; else echo "whets test failed";exit 1; fi
              ;;
          lpack)  if [ -f ${BASE_DIR}/Linpack.txt ];then mv ${BASE_DIR}/Linpack.txt ${BASE_DIR}/log/host/; else echo "linpack test failed";exit 1; fi
              ;;
        esac
      fi
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "docker" &>/dev/null);then
      ...
      echo "test_case: ${test_case// /-}"
      show_test_cmd  " [ docker run -t --memory=${MEMORY_SIZE}m --cpuset-cpus=${CPU_SET// /,} ${DOCKER_IMAGE} bash -c \"/root/${TEST_CMD} ${TEST_METHOD}\" ]${WHITE}"
      if [ "${DRY_RUN}" != "true" ];then
        docker run -t --memory=${MEMORY_SIZE}m --cpuset-cpus=${CPU_SET// /,} ${DOCKER_IMAGE} bash -c "/root/${TEST_CMD} ${TEST_METHOD}"
      fi
    ...
    if (echo "${TEST_TARGET[@]}" | grep -w "hyper" &>/dev/null);then
      ...
      if [ ${USE_HYPER_RUN} == "true" ];then
        show_test_cmd  " [ sudo hyper run --cpu=${CPU_NUM} --memory=${MEMORY_SIZE} ${DOCKER_IMAGE} bash -c \"cd /root;./${TEST_CMD} ${TEST_METHOD}\" ]${WHITE}"
        if [ "${DRY_RUN}" != "true" ];then
          sudo hyper run --cpu=${CPU_NUM} --memory=${MEMORY_SIZE} ${DOCKER_IMAGE} bash -c "cd /root;./${TEST_CMD} ${TEST_METHOD}"
        fi
      else
        CONTAINER_ID=$(hyper_get_container_id)
        if [ "${CONTAINER_ID}" == " " ];then
        ...
        show_test_cmd  " [ sudo hyper exec ${CONTAINER_ID} bash -c \"cd /root;./${TEST_CMD} ${TEST_METHOD}\" ]${WHITE}"
        if [ "${DRY_RUN}" != "true" ];then
          sudo hyper exec ${CONTAINER_ID} bash -c "cd /root;./${TEST_CMD} ${TEST_METHOD}"
        fi
      fi
...
usage:
    ./bench.sh init      #build Dockerfile for hyper:dhrystone
  or
  ...
  or
    ./bench.sh run "host docker hyper" "dhry2 whets lpack"
  or
    ./bench.sh dry-run "host docker hyper" "dhry2 whets lpack"
...
if [ $# -eq 1 ]; then
  if [ "$1" == "init" ];then
    ...
    echo "[ full test ]"
    start_test "host docker hyper" "dhry2 whets lpack"
  elif [ "$1" == "dry-run" ]; then
